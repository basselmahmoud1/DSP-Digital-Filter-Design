function plotMagnitudeResponse(num, den, Fs)
    ...
    % Subplot 1: Magnitude
    subplot(2,1,1);
    plot(f_shifted, mag_dB, 'b', 'LineWidth', 2);
    ...
    % Subplot 2: Phase (wrapped)
    subplot(2,1,2);
    plot(f_shifted, phase_deg, 'r', 'LineWidth', 2);
    ...
    if ~isempty(zeros_vec)
        ...
        plot(real_zeros, imag_zeros, 'bo', 'MarkerSize', 8, ...
    ...
    if ~isempty(poles_vec)
        ...
        plot(real_poles, imag_poles, 'rx', 'MarkerSize', 10, ...
             'LineWidth', 2);
    end
    ...
    text(0.8, 1.1, sprintf('Zeros: %d', length(zeros_vec)), ...
         'FontSize', 10, 'BackgroundColor', 'w', 'Color', 'k');
    text(0.8, 1.0, sprintf('Poles: %d', length(poles_vec)), ...
         'FontSize', 10, 'BackgroundColor', 'w', 'Color', 'k');
    
    hold off;

    %% GROUP DELAY (NORMALIZED OVER π)

    figure('Position', [1000 100 800 400], 'Name', 'Group Delay Response');
    ...
    % Plot group delay
    h1 = plot(w_normalized, gd_shifted, 'g', 'LineWidth', 2);
    hold on;  % <-- FIXED: Added hold on
    ...
    figure('Position', [1000 550 900 400], 'Name', 'Impulse Response');
